     ...
     stages {
         ...
         stage('Docker Build') {
             steps {
                 script {
                    if (env.BRANCH_NAME == 'main') {
                        sh "docker build --build-arg 'PROFILE=prod' -t tastyjapan ."
                    } else {
                        sh "docker build --build-arg 'PROFILE=dev' -t dev-tastyjapan ."
                    }
                 }
             }
         }
         ...
         stage('Aws ECR Upload') {
             steps {
                 script {
                     if (env.BRANCH_NAME == 'main') {
                         sh "docker tag tastyjapan:latest ${env.PROD_ECR}:latest"
                         sh "docker push ${env.PROD_ECR}:latest"
                     } else {
                         sh "docker tag dev-tastyjapan:latest ${env.DEV_ECR}:latest"
